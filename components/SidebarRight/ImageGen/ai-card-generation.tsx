...
export default function AICardGeneration() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // const enhancedPrompt = await promptEnhancer(
      //   `${prompt} in ${settings.style} style, ${settings.backgroundColor} background, ${settings.lighting} lighting, ${settings.pose} pose, ${settings.aspectRatio} aspect ratio`,
      // );
  ...
  return (
    <div className="group relative overflow-hidden w-full max-w-sm bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl transition-all duration-300 hover:shadow-[0_8px_30px_rgb(0,0,0,0.12)] dark:hover:shadow-[0_8px_30px_rgb(0,0,0,0.2)] min-h-[500px] flex flex-col justify-between gap-2">
      <HeaderGeneration title={title} description="Generate images using AI" />
      ...
      <div className="flex-1 overflow-scroll flex flex-col">
        ...
        {showForm ? (
          ...
          <div className="p-4">
            <Preview isLoading={isLoading} imageUrl={images[0]} />
